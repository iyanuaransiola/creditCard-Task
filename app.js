...
function  transformStrArr(string){
        let stringToIntegers = Array.from(string.replace(/\s/g, ""));
        
        let arr =[];

        for(i = 0; i < stringToIntegers.length; i++){        
                arr.push(parseInt(stringToIntegers[i]))
                
        }
        return arr;
}
console.log(transformStrArr(notCertain2));
...
function validateCard(arr){
        ...
        let total = [];
              ...
              let doub = double.map(x => x*2)
               console.log(doub)

               let newDoub = doub.map(x =>{
                       if(x > 9){
                               return x - 9
                       } 
                       else{
                         ...
                         let newDouble = newDoub.reduce((x,y) => {
                               return x+y
                        })
